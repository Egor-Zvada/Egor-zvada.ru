...
$visibleSkills = array_values(array_filter($skills, fn($skill) => empty($skill['is_hidden'])));
$hiddenSkills = array_values(array_filter($skills, fn($skill) => !empty($skill['is_hidden'])));
$totalSkills = count($skills);
$visibleCount = count($visibleSkills);
?>
    ...
    <div class="skills-panel" data-skills-panel>
      ...
      <div class="skills-grid" data-expandable="skills">
        ...
        <?php foreach ($skills as $index => $skill): ?>
          <article class="skill-card <?= !empty($skill['is_hidden']) ? 'is-hidden' : '' ?>" data-expandable-item>
        ...
        <button class="skill-card skill-card--control skill-card--more" type="button" data-expand-toggle="skills" data-open-label="Показать больше" data-close-label="Скрыть" aria-expanded="false">
          ...
          <span class="skill-card__control-text">Открыть ещё <?= count($hiddenSkills) ?> направлений и инструментов</span>
          <span class="skill-card__arrow" aria-hidden="true">→</span>
        </button>
      </div>
    </div>
